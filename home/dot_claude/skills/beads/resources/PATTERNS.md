...
```bash
# 1. Create the bug, linked to the work that surfaced it
bd create "Fix: payment validation bypasses card expiry check" \
  -t bug -p 0 --deps discovered-from:checkout-1 --json

# 2. Block the current work on it
bd update checkout-1 --status blocked \
  --notes "Blocked by payment-bug-2: security hole in validation" --json

# 3. Claim the new work atomically
bd update payment-bug-2 --claim --json
```
...
2. **Check what's stuck**: `bd blocked --json` to understand blockers
3. **Check recent progress**: `bd list --status closed --json` to see completions
4. **Read detailed context**: `bd show <id> --long --json` for the issue you'll work on
5. **Claim issue**: `bd update <id> --claim --json` for atomic start
