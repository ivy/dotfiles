...
Everything here goes through the `bd` CLI. There is no beads MCP server in this
environment, so no `mcp__*beads*` tool exists — if you reach for one, use the equivalent
`bd` subcommand instead.

- Check health first: `bd doctor`
- Embedded mode (the default) accesses the database directly and has a 3-5 second sync delay
- Server mode routes through a Dolt SQL server; see [Status Updates Not Visible](#status-updates-not-visible)
...
**Symptom:** dependencies end up reversed from what you intended.

`bd dep add` takes the waiting issue first:
...
**Mnemonic:**
- First argument: the issue that **waits**
- Second argument: the issue that **must finish first**
...
| Dependencies backwards | `bd dep add <waiting-issue> <blocker>` — waiting issue first |
