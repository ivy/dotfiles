...
```bash
bd --version  # Requires v1.2.2+
```

- **bd CLI** installed and in PATH
- **Workspace**: run `bd where` to locate it. This is the authoritative path — never
  infer it from the current directory.
- **Initialization**: a human runs `bd init` once per repository. If `bd where` reports
  no workspace, that repository is not opted in — ask before initializing one.
...
If `bd --version` reports newer than `1.2.2`, this skill may be stale. Run `bd prime` for current CLI guidance — it auto-updates with each bd release and is the canonical source of truth ([ADR-0001](adr/0001-bd-prime-as-source-of-truth.md)).
