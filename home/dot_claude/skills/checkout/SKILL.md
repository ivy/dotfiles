...
### 2. Derive Branch Name (from issue)
...
| Context | Approach |
|---------|----------|
| On default branch, clean state | `git checkout -b <branch> origin/<default>` |
| On another feature branch | Consider `EnterWorktree` for isolation, or `git checkout -b` if user prefers simplicity |
