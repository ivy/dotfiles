...
The template reads `[data.claude] use_bedrock` from chezmoi config (set during `chezmoi init` or via `USE_BEDROCK` env var). When false, it emits `model: <tier>`. When true, the entire `model:` line is omitted — skills inherit the session model, avoiding the Bedrock billing header leak.
