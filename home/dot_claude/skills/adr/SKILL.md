...
**Autonomy:** model-invocable · acts autonomously — the ADR is a local file write; the pressure-testing before drafting is the skill's purpose, not a commit gate
...
If no ADR directory exists, create `docs/adrs/`.
...
5. Under **Rejected Options**, cover only the options that lost. The chosen option is already described by Decision Outcome and Consequences; restating it there is a bug, not thoroughness
...
- Restating the chosen option under Rejected Options
