...
**Autonomy:** model-invocable · acts autonomously — stages, commits, and amends unpushed commits without confirmation · has no push capability
...
**Never use `git -C <path>`** — always run git commands from the current working directory. The `-C` flag rewrites the command so it no longer matches the `allowed-tools` patterns, giving up their pre-approval. Plain `git status`, `git log`, etc. already operate on the repo you're in.
